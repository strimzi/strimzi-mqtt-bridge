...
Note the GitHub Actions Run ID from the URL (e.g., `https://github.com/strimzi/strimzi-mqtt-bridge/actions/runs/1234567890` - the Run ID is `1234567890`).
...
In case of a CVE in the base container image, we might need to rebuild the MQTT Bridge container image.
...
4. Fill in the required parameters:
   * **Release Version**: for example `1.2.0`
   * **Release Suffix**: for example `2` (used to create the suffixed images such as `strimzi/mqtt-bridge:1.2.0-2` to identify different builds done for different CVEs)
